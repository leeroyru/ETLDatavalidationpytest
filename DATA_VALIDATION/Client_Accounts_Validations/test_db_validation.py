...
def test_column_values_validation(validated_data, column):
    ...
    if column == 'account_number':
        assert all(source_valid['account_number'].str.len() == 4), "account_number in source must be exactly 6 characters."
...
def test_discrepancies_validation(validated_data):
    ...
    # Check for discrepancies between source and target values
    for col in ['account_number', 'account_status', 'note_type', 'updated_by', 'designation']:
        # Create a boolean mask for discrepancies
        mask = source_valid[col] != target_valid[col]
        assert not mask.any(), f"Discrepancies found in column '{col}' between source and target data."
