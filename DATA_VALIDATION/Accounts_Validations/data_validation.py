...
def validate_accounts():
    ...
    SELECT
        ...
        ext.[designation]
    FROM
        [ETL_TestDB].[dbo].[accounts] AS acc
    LEFT JOIN
        [ETL_TestDB].[dbo].[AccountNotes] AS notes ON acc.[account_number] = notes.[account_number]
    LEFT JOIN
        [ETL_TestDB].[dbo].[AccountExternal] AS ext ON acc.[account_number] = ext.[account_number]
    WHERE
    ...
    discrepancies = pd.merge(source_valid, target_valid, on='account_number', how='outer', indicator=True)
